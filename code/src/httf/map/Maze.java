	...
	private void generateCrazyLabyrinth() {
		...
		for (int x = 0; x < level.pixels.length; x += 3) {
			for (int y = 0; y < level.pixels[x].length; y += 3) {
				level.pixels[x][y] = 0x0000FF;
				level.pixels[x][y + 2] = 0x0000FF;
				level.pixels[x + 2][y] = 0x0000FF;
				level.pixels[x + 2][y + 2] = 0x0000FF;
				level.pixels[x + 1][y + 1] = 0xFFFFFF;
				
				if(rand.nextBoolean()) {
					level.pixels[x + 1][y] = 0xFFFFFF;
				} else {
					level.pixels[x + 1][y] = 0x000000;
				}
				/*
				if (rand.nextBoolean()) {
